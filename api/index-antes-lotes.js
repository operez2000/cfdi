...
const recuperarCfdi = async (params) => {
  ...
  */
  let url = `https://facturacion.itimbre.com/service.php?q=${params}`
...
function AfectaBaseDatos(datos) {

  console.log('datos para afectar BD', datos)
    
    /* Para afectar directamente en el servidor 
    conn.connect();

    var userid = 'JP';
    var qry = "SELECT id, userid FROM usuarios";
    qry = "UPDATE usuarios SET direccion = 'Prueba dir...' WHERE userid = '" + userid + "'";
console.log('qry', qry);
console.log('----------------');
  ...
  // let url = 'http://127.0.0.1:8888/posplus/server/lotes.php?opcion=graba'; 
  // let url = 'http://code-ware.com/proyectos/gusher/server/lotes.php?opcion=graba';
  let url = 'http://74.208.34.182:3002/lotes/?opcion=graba';
  console.log('url', url)
  axios({
    method: 'get',
    url,
    ...
    console.log('Final en AfectaBaseDatos()');
  });
...
app.get('/', mdi, (req, res) => {
  ...
  try {
    // La fecha viene de lotes como YYYY-MM-DD, la cambio a Mmm/DD/YYYY para enviar por correo
    caducidad = arrMeses[ parseInt(req.query.cad.substr(5, 2)) - 1 ] + '/' + 
                req.query.cad.substr(8, 2) + '/' + 
                req.query.cad.substr(0, 4);

    console.log('--------------------------------------------');
    res.send('Notificacion...');

    req.query.lote = req.query.lote.replace(/\!/g, " ");

    body = "<h3>Notificación para dar de alta Lote</h3>" +
      "<hr>" +
      "<ul>" +
      "    <li>Etiqueta: " + req.query.eti + "</li>" +
      "    <li>Código interno: " + req.query.cod + "</li>" +
      "    <li>Lote: " + req.query.lote + "</li>" +
      "    <li>Caducidad: " + caducidad + "</li>" +
      "    <li>Sucursal: " + req.query.suc + "</li>" +
      "</ul>";

    correos = 'suc.monarca@gusher.com.mx, suc.otay@gusher.com.mx, suc.palmas@gusher.com.mx, suc.rio@gusher.com.mx, suc.rosarito@gusher.com.mx, ' +
              'silvia.ochoa@gusher.com.mx, silviaordaz@gusher.com.mx, alejandraochoa@gusher.com.mx';

    //  correos = "operez2000@gmail.com";
    
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
    var transporter = nodeMailer.createTransport({
      service: 'gmail',
      auth: {
        user: 'gusher.sistemas@gmail.com',
        pass: 'gsistemas123!'
      }
    });

    const mailOptions = {
      from: '"Gusher Sistemas" <gusher.sistemas@gmail.com>', // sender address
      to: correos, // list of receivers
      subject: 'Notificación de Lote ' + req.query.eti, // Subject line
    ...
    transporter.sendMail(mailOptions, function(err, info) {
      if (err) {
        ...
        console.log("Rechazado:", info.rejected);
      }
      console.log('============================================');
    });
    */
    // res.end("Respuesta correcta...")
    AfectaBaseDatos(req.query);    
  } catch (error) {
    console.log(error)
  }
...
app.post("/cancelarFactura", mdi, (req, res) => {
  ...
  axios({
    ...
    let retcode
    if (response.result.retcode != undefined) {
      retcode = response.result.retcode
    } else {
      retcode = -1
    }
    if (retcode == 1) { // Todo bien, se timbró la facura
...
app.post("/timbra-global", async (req, res) => {
  ...
  const url = `https://facturacion.itimbre.com/service.php?q=${params}`
  ...
  try {
    ...
    console.log('error', error)
    let errorDescrip = 'Error indefinido'
    if (error.stack) {
      errorDescrip = error.stack.replace(/\s+/gm, ' ')
    }
    if (error.message) {
      errorDescrip += " / " + error.message
    }
    response.result = {
      retcode: -1,
