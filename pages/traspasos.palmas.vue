    ...
    async generarPdf () {
      ...
      try {
        ...
        destinoIds.forEach((destId) => {
          ...
          const estimatedPages = Math.max(1, Math.ceil((items.length + 8) / 25))
          pageRanges.push({
            destId,
        ...
        const docDefinition = {
          ...
          content,
          footer: (currentPage) => {
            const section = pageRanges.find(range => currentPage >= range.startPage && currentPage < range.startPage + range.pageCount)
            const sucursal = section
              ? this.sucursales.find(s => s.id === parseInt(section.destId, 10)) || {}
              : {}
