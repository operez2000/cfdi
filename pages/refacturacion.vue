      ...
      <v-row>
        ...
        <v-col md="2">
          <v-text-field
            ...
            hint="No. de Factura a Cancelar"
            ref="refFacturaOriginal"
          >
            <template #append>
              <v-btn
                icon
              ...
              <v-row>
                ...
                <v-col md="auto">
                  <v-btn
                    ...
                    @click="onClick('ACTUALIZAR_CLIENTE'); tab = 'factura'"
              ...
              <v-row class="mt-2">
                ...
                <v-col md="auto">
                  <v-btn
                    ...
                    :disabled="Number(facturaOriginal.detalle.Total) == 0 || cliente.nuevo || recibo.cancelada == 'S' || loaders.generaFactura"
                  >
    ...
    validaUsuario() {
      ...
      if (this.usuario && this.usuario.length > 0 && this.clave && this.clave.length > 0) {
        ...
        this.$axios({
          ...
          setTimeout(() => {
            this.$refs.refFacturaOriginal.focus()
          }, 100)
    ...
    onClick(origen, opcion = "rfc", valor = "") {
      if (origen == "RECUPERAR_FOLIO") {
        ...
        this.$axios({
          ...
          if (resp.data.response == 200) {
            if (resp.data.data.tipo == "G") { // Global
              ...
              this.factura.comentarios = `Caja-Folio: ${this.recibo.cajaFolio} | ${this.recibo.fecha} | Cajero ${this.recibo.cajero} | Vendedor ${this.recibo.vendedor} | Cuenta ${this.recibo.cuenta}`
            }
          } else {
        ...
        this.$axios({
          ...
          console.log("/api/facturar resp.data", resp.data)
          if (resp.data.result.retcode == -1) {
          ...
          if (data.pdfBase64 != "") {
            ...
            //link.href = 'data:application/octet-stream;base64,' + resp.data.result.pdfBase64
            //link.click()
            //link = null
          }
        }).catch(error => {
...
*/
    ...
    XmlParse() {
      try {
        ...
        // En caso de que exista el nodo Descuento
        try {
          data.Descuento = xmlDoc.getElementsByTagName("cfdi:Comprobante")[0].getAttribute("Descuento")
        } catch (error) {
        ...
        for (let index = 0;index < children.childElementCount; index++) {
          ...
          detalle.push({
            ...
            total: (Number(child.getAttribute("Importe")) - Number(descuento)).toFixed(2),
            ObjetoImp: "02",
            Impuestos: {
              Traslados: [
                {
            ...
            totalNeto: (Number(child.getAttribute("Importe")) - Number(descuento) + Number(traslados[index].childNodes[0].getAttribute("Importe")) ).toFixed(2)
          })
          // En caso de que haya descuento agrego el elemento
          if (descuento != "0.00") {
            detalle[detalle.length -1].Descuento = descuento
          }
        ...
        data.Impuestos = {
          ...
          TotalImpuestosTrasladados: data.TotalImpuestosTrasladados.toFixed(2),
          Traslados: []
        }
        // Gravable
    ...
    creaEstructura () {
      ...
      if (this.facturaOriginal.data.Descuento) {
        jsonCfdi.datos_factura.Descuento = this.facturaOriginal.data.Descuento
      }
    ...
    cancelaFactura() {
      ...
      this.$axios({
        ...
        data: {
          ...
          serie: this.serie,
          folio: this.facturaOriginal.folio
        }
      }).then(resp => {
